...
import faceRecognition.face_recognition
import cv2
import os
...
def get_face_embedding(model_path,files_list, names_list):
    ...
    colorSpace="RGB"
    # 初始化mtcnn人脸检测
    face_detect = faceRecognition.face_recognition.Facedetection()
    # 初始化facenet
    face_net = faceRecognition.face_recognition.facenetEmbedding(model_path)
...
def create_face_embedding(model_path,dataset_path,out_emb_path,out_filename):
    ...
    :return: None
    '''
    files_list,names_list=file_processing.gen_files_labels(dataset_path,postfix=['*.jpg', '*jpeg'])
