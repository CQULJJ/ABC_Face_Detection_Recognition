...
def cv_show_image(title, image, type='rgb'):
    ...
    # cv2.resizeWindow(winname="人脸识别",width=480,height=640)
...
def read_image1(bgr_image, resize_height=None, resize_width=None, normalization=False,colorSpace='RGB'):
    '''
    读取图片数据,默认返回的是uint8,[0,255]
    :param filename:
    :param resize_height:
    ...
    # bgr_image = cv2.imread(filename,cv2.IMREAD_IGNORE_ORIENTATION|cv2.IMREAD_COLOR)
    if bgr_image is None:
        print("frame Warning:不存在:{}")
        return None
    if len(bgr_image.shape) == 2:  # 若是灰度图则转为三通道
        print("Warning:gray image")
        bgr_image = cv2.cvtColor(bgr_image, cv2.COLOR_GRAY2BGR)

    if colorSpace=='RGB':
        bgr_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)  # 将BGR转为RGB
    elif colorSpace=="BGR":
        bgr_image = bgr_image
    else:
        exit(0)
    # show_image(filename,image)
    # image=Image.open(filename)
    image = resize_image(bgr_image,resize_height,resize_width)
    image = np.asanyarray(image)
    if normalization:
        image=image_normalization(image)
    # show_image("src resize image",image)
...
def show_image_bboxes_text(title, rgb_image, boxes, boxes_name):
    ...
    for name ,box in zip(boxes_name,boxes):
        ...
        rgb_image=change_cv2_draw(bgr_image, name, (box[0], box[1]), 20,(255, 0, 0))
...
def change_cv2_draw(image,strs,local,sizes,colour):
    ...
    font = ImageFont.truetype("msyh.ttc",sizes, encoding="utf-8")
...
if __name__=="__main__":
    ...
    image = read_im
    age_gbk(image_path, resize_height=None, resize_width=None)
